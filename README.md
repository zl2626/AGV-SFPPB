...
控制器增加了一个默认开启的 `learning_enabled` 参数，用于在完全相同的车辆、SFPPB、初始 Actor 和安全滤波条件下冻结或开启 Identifier–Critic–Actor 更新。Simulink 默认参数为 `100,1`，即 `lambda = 100` 且在线学习开启。

60 s 对照结果：

| 模式 | 最小归一化裕量 | `max|delta|` | 滤波介入比例 | 冲突次数 | 控制能量 | y RMS | phi RMS | `||Wa-Wa0||` |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 冻结学习 | `14.5908%` | `0.493377` | `7.2476%` | 0 | `0.698091` | `0.00755483` | `0.00129728` | `0` |
| 在线学习 | `8.3474%` | `0.497136` | `7.2802%` | 0 | `0.697138` | `0.00813220` | `0.00129621` | `0.0652404` |

当前在线学习将控制能量降低约 `0.14%`，并使 phi RMS 略降约 `0.08%`，但 y RMS 增大约 `7.6%`，边界鲁棒裕量也明显下降。因此现阶段只能证明网络发生了在线更新，尚不能证明 RL 带来净性能提升。默认主线仍保留在线学习，用于继续研究二维 HJB/Actor 结构；论文结论中必须保留这一限制，下一步应优先改进 Actor 更新或代价分配，而不是继续调整安全滤波 `lambda`。
...
4. 运行 `AGV_diagnose(20,false,100,true)` 获取 Identifier、Bellman residual、Actor/安全滤波和权重数值诊断；第三个参数用于测试 `lambda`，第四个参数用于开启或冻结在线学习。
