...
最新加权投影诊断冻结 `AGV_transfor.m`，只在 `AGV_ctrl.m` 中使用 `P=diag([3,1])`，将 `z2_control` 改为 `C'*P*z2/sqrt(C'*P*C)-O`。该实验用于检查 y/phi 通道在普通投影中互相抵消的问题，不作为最终二维 HJB/Actor 公式。
...
- 最新诊断版本只在 `AGV_ctrl.m` 的导数和输出计算中加入 `P=diag([3,1])` 加权投影；其余控制参数和学习率保持不变。
...
- `P=diag([3,1])` 加权投影版本可以运行到约 `t=7.062 s`，说明二维误差投影抵消确实是早期 y 撞边界的重要原因。

## 当前存在的问题

当前 SFPPB-RL 控制器仍然没有通过 20 s 全程仿真。需要区分五个版本的失效现象：
...
- 最新加权投影版本在约 **t=7.063 s** 失败；在 `t=7.062 s` 时 y 上边界余量仍约 `2.49×10^-2`，但 `e_phi≈0.0049379`、phi 上界为 `0.005`，余量仅约 `6.21×10^-5`；
- 加权投影实验在 `t=7.062 s` 时 `delta≈-0.15598 rad`，全程范围约 `[-0.15609,0.03003] rad`，未发生输入饱和；`z1≈[0.344,5.076]^T`，`z2≈[0.1397,0.2024]^T`；
- 该结果证明加权投影可以消除早期 y/phi 抵消并显著延长运行时间，但固定 `P=diag([3,1])` 又把瓶颈转移到 phi 通道，因此它只能作为诊断证据，不是最终控制律；
...
当前加权投影实验版本预期会在约 7.063 s 附近暴露上述稳定性问题；具体时刻会随 MATLAB/Simulink 求解器步长略有变化。这属于待解决问题的复现实验，不是最终结果。

## 下一步工作

1. 根据加权投影诊断结果，重新推导二维 `z2` 到单转向输入的 HJB/值函数梯度接口，避免直接使用 `C'*z2` 丢失通道优先级信息。
2. `P=diag([3,1])` 仅作为诊断；在最终二维 HJB/Actor 接口明确前，不通过继续调固定 P、`c2` 或 Actor 学习率来掩盖问题。
